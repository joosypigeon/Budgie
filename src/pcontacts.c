...
static void pc_resolveInterpenetration(ParticleContact *self, buReal duration)
{
    ...
    if (self->_particle[1]) {
        ...
        INSTANCE_METHOD_AS(ParticleVTable, self->_particle[1], setPosition, (buVector3){0.0, 0.0, 0.0});
    }
}
...
static const char *get_name(ParticleContactClass *cls) {
...
void ParticleContactCreateClass() {
    ...
    if (!pc_initialized) {
        ...
        particleContactClass.get_name = get_name;
...
static void pcr_resolveContacts(
    ...
    while(self->_iterationsUsed < self->_iterations) {
        ...
        for (i = 0; i < numContacts; i++) {
            buReal sepVel = INSTANCE_METHOD_AS(ParticleContactVTable, &contactArray[i], calculateSeparatingVelocity);
            if (sepVel < max &&
...
static const char *get_name(ParticleContactResolverClass *cls) {
    return cls->class_name;
}
...
void ParticleContactResolverCreateClass() {
    ...
    if (!pcr_initialized) {
        ...
        particleContactResolverClass.get_name = get_name;
